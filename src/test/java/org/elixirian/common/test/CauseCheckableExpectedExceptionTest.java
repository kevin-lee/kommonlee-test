...
public class CauseCheckableExpectedExceptionTest
{
	...
	public final void testCauseCheckableExpectedException()
	{
		...
		/* given */
		final CauseCheckableExpectedException causeCheckableExpectedException =
			new SubCauseCheckableExpectedException();
	...
	public final void testExpectMatcherOfT() throws Throwable
	{
		...
		when(matcher.matches(nullPointerException)).thenReturn(Boolean.TRUE);
		final Statement base = mock(Statement.class);
		doThrow(nullPointerException).when(base)
				.evaluate();
		...
		verify(base).evaluate();
		verify(matcher).matches(nullPointerException);
	}
	...
	public final void testExpectMessage() throws Throwable
	{
		...
		final Matcher<Object> matcher = mock(Matcher.class);
		when(matcher.matches(any())).thenReturn(Boolean.FALSE);
		when(matcher.matches(expectedMessage)).thenReturn(Boolean.TRUE);
		final Statement base = mock(Statement.class);
		doThrow(new UnsupportedOperationException(expectedMessage)).when(base)
				.evaluate();
	...
	public final void testExpectCauseMatcherOfT() throws Throwable
	{
		...
		when(matcher.matches(rootException)).thenReturn(Boolean.TRUE);
		final Statement base = mock(Statement.class);
		doThrow(nestedException).when(base)
				.evaluate();
		...
		assertThat(statement, is(instanceOf(CauseCheckableExpectedExceptionStatement.class)));
		verify(base).evaluate();
		verify(matcher).matches(nestedException.getCause());
	}
	...
	public final void testExpectCauseMessageContains() throws Throwable
	{
		/* given */
		final Statement base = mock(Statement.class);
		doThrow(new NestedException(new RootException("This is a test exception."))).when(base)
				.evaluate();
	...
	public final void testExpectCauseMessage() throws Throwable
	{
		...
		when(matcher.matches(expectedMessage)).thenReturn(Boolean.TRUE);
		final Statement base = mock(Statement.class);
		doThrow(new NestedException(new RootException(expectedMessage))).when(base)
				.evaluate();
